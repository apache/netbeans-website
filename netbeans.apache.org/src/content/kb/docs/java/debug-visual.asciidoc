...
1. To start a debugging sesson, press:
[cols="1,4"]
|===
|*Windows*(TM)/Linux |kbd:[Ctrl-F5]
|*macOS*(TM) |kbd:[⌘-F5 on Mac]
|===

or, click the Debug button in the toolbar (Ctrl-F5; ⌘-F5 on Mac) to start the debugging session. Alternatively, right-click the project node in the Projects window and choose Debug. When you start the session, the IDE will launch the Anagram Game application and open the Debugging window.
