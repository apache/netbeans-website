...
TIP: To use JDK 14 features in the Java Editor, you'll need to run Apache NetBeans 11.3 itself on JDK 14, since the javac  that Apache NetBeans runs on is used by the editor for features such as syntax coloring. Also, make sure to uninstall nb-javac, if it is installed, so that the javac from JDK 14 will be used for these reasons.
