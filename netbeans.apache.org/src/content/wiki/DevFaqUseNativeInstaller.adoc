...
link:http://bits.netbeans.org/dev/javadoc/org-netbeans-modules-autoupdate-services/overview-summary.html[Autoupdate Services] primary handles NetBeans plugins based on xref:DevFaqWhatIsNbm.adoc[NBM] packaging. 
...
Using xref:DevFaqModulesGeneral.adoc[META-INF/services] or (in NetBeans 6.9) the `@ServiceProvider` annotation (see link:https://blogs.oracle.com/geertjan/entry/meta_inf_services_vs_layer[Geertjan's blog for more info]):
