...
A good example of this is in the link:https://netbeans.apache.org/tutorials/nbm-povray-1.html[POV-Ray tutorial].  It launches an external process that generates a `.png` file.  When the process ends, it wants to open it, so it does the following:
