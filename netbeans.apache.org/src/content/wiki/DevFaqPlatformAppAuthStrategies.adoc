...
However, as described in the link:http://bits.netbeans.org/dev/javadoc/org-openide-modules/architecture-summary.html[Module System documentation], you can use the `netbeans.mainclass` xref:DevFaqPlatformRuntimeProperties.adoc[system property] to specify a different class to run at startup.  
