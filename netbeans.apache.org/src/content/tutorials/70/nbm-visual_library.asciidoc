...
[start=4]
1. 
Select "Generate XML Layer", but do not select "Generate OSGi Bundle". Leave the locations of both the localizing bundle and the XML layer file so that they will be stored in a package with the name  ``org/netbeans/shapesample`` . Click Finish.
...
[start=2]
1. In the same way as explained in step 3 of the section called "Getting Started", earlier in this tutorial, add dependencies on the Actions API, Lookup API, Nodes API, and Common Palette API.
