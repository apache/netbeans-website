...
[start=7]
1. The code editor should now be open over a class called `OpenEditorAction`, which subclasses `CallableSystemAction` (a NetBeans subclass of `javax.swing.Action` which lets you associate context sensitive help with an action). Add the following code to its `performAction()` method:
...
[start=1]
1. Expand the Important Files node of the My API project

[start=2]
1. Double click the Module Manifest node

[start=3]
1. Delete the following line from the manifest:
...
[start=4]
1. Then delete the corresponding `layer.xml` file in `org.myorg.myapi`
