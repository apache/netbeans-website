...
NOTE: This document uses NetBeans Platform 7.1 and NetBeans IDE 7.1. If you are using an earlier version, see  link:../70/nbm-selection-2.html[the previous version of this document].
...
image::images/selection-2_71_add-deps-beantreeview-71.png[]
...
[start=3]
1. 
Right-click inside the `TopComponent`, and choose Set Layout > Border Layout, as shown below:


image::images/selection-2_71_border-layout-71.png[]
...
[start=5]
1. 
Select your `JScrollPane` and open the Properties Window (Ctrl-Shift-7). Use the Code tab to quickly customize the creation code, by adding `new BeanTreeView()` as shown below:


image::images/selection-2_71_custom-creation-71.png[]
...
[start=10]
1. Now, back in the source editor, press Ctrl-Shift-I to Fix Imports. You will be notified that `EventChildFactory` could not be resolved. That's okay—you're about to write it.
...
[start=2]
1. In the New Java Class wizard, name the class "EventChildFactory", and click Finish or press Enter to create the class.
...
You are now ready to run, so right-click `EventManager` and choose Clean and Build, and then right-click again and choose Run from the popup menu. When NetBeans starts, use your Open Editor action on the Window menu to open an instance of `MyEditor`.


image::images/selection-2_71_result-1.png[]
...
image::images/selection-2_71_result-2.png[]
...
image::images/selection-2_71_result-3.png[]
