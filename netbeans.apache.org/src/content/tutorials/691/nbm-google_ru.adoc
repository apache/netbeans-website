...
image::images/google_60_add-module-dependency.png[] Нажмите кнопку "ОК", после чего нажмите кнопку "ОК" еще раз для закрытия диалогового окна "Свойства проекта".
...
image::images/google_60_fix-all-imports.png[] Нажмите кнопку "ОК". Для  ``GooglePanel.java``  создаются следующие выражения импорта:
