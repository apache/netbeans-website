...
mvn -DarchetypeGroupId=org.codehaus.mojo.archetypes -DarchetypeArtifactId=netbeans-platform-app-archetype -DarchetypeVersion=1.20 -DgroupId=com.mycompany -DartifactId=mavenPlatformApp -Dversion=1.0-SNAPSHOT -Dpackage=com.mycompany -DnetbeansVersion=RELEASE81 archetype:generate
----
...
If you look at the  ``<properties>``  element, you can see that the version of the NetBeans Platform is specified as  ``RELEASE81`` . In the POM of your sub-modules, you can specify  ``${netbeans.version}``  as the version and the module will use the version specified in the parent POM project. The enables you to easily update the NetBeans Platform version for all the project's sub-modules.
...
mvn -DarchetypeGroupId=org.codehaus.mojo.archetypes -DarchetypeArtifactId=nbm-archetype -DarchetypeVersion=1.15 archetype:generate
----
...
[INFO] Parameter: package, Value: com.mycompany.mavenplatformmodulea
[INFO] Parameter: version, Value: 1.0-SNAPSHOT
[INFO] Parameter: netbeansVersion, Value: RELEASE81
[INFO] Parameter: groupId, Value: com.mycompany
[INFO] Parameter: artifactId, Value: mavenPlatformModuleA
...
[start=3]
1. Edit the new module's  ``pom.xml``  to make the following changes (marked as *1*) to add a dependency on the modules you are referring to in the code above.
