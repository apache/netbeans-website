...
In this tutorial, you will be introduced to aspects of the new NetBeans 6.0 "Retouche" APIs, which give you access to the NetBeans Java editor.

NOTE: This is not the latest version of this document. It applies to NetBeans IDE 6.0/6.1 only.  link:../nbm-copyfqn.html[Click here] to see the most up to date version.
...
[start=2]
1. Choose Conditionally Enabled, and keep all the defaults, as shown below, so that the action will be sensitive to  ``DataObjects``  and so that it will only be enabled when one  ``DataObject``  is selected.
